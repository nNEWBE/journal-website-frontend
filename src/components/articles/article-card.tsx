...
export function ArticleCard({
  ...
  if (variant === "editorial") {
    return (
      <article className="bg-white border border-slate-200/90 p-5 sm:p-6 shadow-2xs hover:border-slate-300 transition-all group">
          ...
          <div className="flex flex-col justify-between h-full min-w-0">
            <div>
              ...
              <Link href={`/articles/${article.slug}`} className="mt-2.5 block">
                <h2 className="font-academic text-lg sm:text-xl font-medium leading-snug text-slate-950 group-hover:text-[#1e40af] transition-colors line-clamp-2">
                  {article.title}
                </h2>
              </Link>
  ...
  return (
    <article className="bg-white border border-slate-200/90 p-5 flex flex-col justify-between shadow-2xs hover:border-slate-300 transition-all group">
      <div>
        <Link
          href={`/articles/${article.slug}`}
          className="relative aspect-[4/3] w-full overflow-hidden bg-slate-950 border border-slate-200/80 block mb-4"
        >
          <Image
            src={imageUrl}
            alt={article.title}
        ...
        <Link href={`/articles/${article.slug}`} className="mt-2 block">
          <h2 className="font-academic text-base font-medium leading-snug text-slate-950 group-hover:text-[#1e40af] transition-colors line-clamp-2">
            {article.title}
          </h2>
        </Link>

        <p className="mt-1.5 text-xs font-medium text-slate-600 line-clamp-1">
          {article.authors.join(", ")}
        </p>
      </div>
